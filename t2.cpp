...
TreeNode* sortedArrayToBST(const vector<int> &A, int start, int end){
    if(start > end)
        return NULL;
    int mid = (start+end)/2;
    TreeNode *temp = new TreeNode(A[mid]);
    temp->left = sortedArrayToBST(A, start, mid - 1);
    temp->right = sortedArrayToBST(A, mid + 1, end);
    return temp;
}
...
TreeNode* BuildTree(const vector<int> &A, int start, int end){
    ...
    TreeNode *temp = new TreeNode(A[maxindex]);
    temp->left = BuildTree(A, start, maxindex - 1);
    temp->right = BuildTree(A, maxindex + 1, end);
    return temp;
}
...
int lca(TreeNode* root, int val1, int val2) {
    TreeNode *result = findlca(root,val1,val2);
    if(result == NULL)
        return -1;
    return result->val;
}
